...
def setup_daq(plot=False):
    ...
    daq = WaveformGeneratorHardware(dev_name, rate, period, ao_channels)
...
def setup_camera():
    ...
    grabber.remote.set("ExposureTime", dwell_time) # set exposure time ms, i.e. slit width
...
def finish_imariswriter(converter):
    ...
    color_infos[0].mRangeMin=0
    color_infos[0].mRangMax=1000
...
def main():
    ...
    try: 
		...
		subprocess.call(['taskkill', '/F', '/T', '/PID',  str(cmd.pid)]) # termiinate memento subprocess
