...
class EGrabberInfo(object):
    ...
    def _from_EGrabberDiscovery_egrabbers_at(cls, discovery, ix):
        ...
        cE.Eur_EGrabberDiscovery_egrabbers__from__int(discovery._box,
            ct.c_int(ix), ct.byref(box))
        return cls(discovery.gentl, box)

    @classmethod
    def _from_EGrabberDiscovery_inferfaceInfo(cls, discovery, interface_index):
        box = cE.Eur_EGrabberInfo()
        cE.Eur_EGrabberDiscovery_interfaceInfo__from__int(discovery._box,
            ct.c_int(interface_index), ct.byref(box))
        return cls(discovery.gentl, box)

    @classmethod
    def _from_EGrabberDiscovery_deviceInfo(cls, discovery, interface_index, device_index):
        box = cE.Eur_EGrabberInfo()
        cE.Eur_EGrabberDiscovery_deviceInfo__from__int__int(discovery._box,
            ct.c_int(interface_index), ct.c_int(device_index), ct.byref(box))
        return cls(discovery.gentl, box)

    @classmethod
    def _from_EGrabberDiscovery_streamInfo(cls, discovery, interface_index, device_index, stream_index):
        box = cE.Eur_EGrabberInfo()
        cE.Eur_EGrabberDiscovery_streamInfo__from__int__int__int(discovery._box,
            ct.c_int(interface_index), ct.c_int(device_index), ct.c_int(stream_index), ct.byref(box))
        return cls(discovery.gentl, box)

    @classmethod
    def _from_EGrabberCameraInfo_egrabbers_at(cls, camera_info, index):
        box = cE.Eur_EGrabberInfo()
        cE.Eur_EGrabberCameraInfo_grabber_at__from__size_t(camera_info._box,
            ct.c_size_t(index), ct.byref(box))
        ...
        try:
            if type(key) is tuple:
                ...
                if ktype in six.integer_types:
                    cE.Eur_EGrabberDiscovery_cameras__from__int__int(discovery._box,
                        ct.c_int(key[0]), ct.c_int(key[1]), ct.byref(self._box))
                elif ktype in (str, six.text_type):
                    cE.Eur_EGrabberDiscovery_cameras__from__const_char_p__int(discovery._box,
                        utils.to_cstr(key[0]), ct.c_int(key[1]), ct.byref(self._box))
                else:
                    raise TypeError('key[0] must be int or str.')
            else:
                ktype = type(key)
                if ktype in six.integer_types:
                    cE.Eur_EGrabberDiscovery_cameras__from__int(discovery._box,
                        ct.c_int(key), ct.byref(self._box))
                elif ktype in (str, six.text_type):
                    cE.Eur_EGrabberDiscovery_cameras__from__const_char_p(discovery._box,
                        utils.to_cstr(key), ct.byref(self._box))
                else:
...
class EGrabberDiscovery(object):
    ...
    def device_count(self, interface_index):
        ...
        """
        c_count = ct.c_int()
        cE.Eur_EGrabberDiscovery_deviceCount__from__int(self._box,
            ct.c_int(interface_index), ct.byref(c_count))
        return c_count.value
    ...
    def stream_count(self, interface_index, device_index):
        ...
        """
        c_count = ct.c_int()
        cE.Eur_EGrabberDiscovery_streamCount__from__int__int(self._box,
            ct.c_int(interface_index), ct.c_int(device_index), ct.byref(c_count))
        return c_count.value
