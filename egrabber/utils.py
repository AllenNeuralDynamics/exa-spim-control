...
class Ctype():
    ...
    def __init__(self, ctype, ctypename=None):
        (ctypename_, self._getter, self._deleter) = self.to_attr[ctype]
        self.ctypename = ctypename if ctypename else ctypename_
        self.box = ctype()

    @classmethod
    def from_info_datatype(cls, info_datatype):
        (ctype, ctypename) = cls.to_ctype[info_datatype]
        return cls(ctype, ctypename)
    ...
    to_ctype = {
        c.INFO_DATATYPE_SIZET   : (ct.c_size_t, 'size_t'),
        c.INFO_DATATYPE_INT16   : (ct.c_int16,  'int16_t'),
        c.INFO_DATATYPE_INT32   : (ct.c_int32,  'int32_t'),
        c.INFO_DATATYPE_INT64   : (ct.c_int64,  'int64_t'),
        c.INFO_DATATYPE_UINT16  : (ct.c_uint16, 'uint16_t'),
        c.INFO_DATATYPE_UINT32  : (ct.c_uint32, 'uint32_t'),
        c.INFO_DATATYPE_UINT64  : (ct.c_uint64, 'uint64_t'),
        c.INFO_DATATYPE_BOOL8   : (c_bool8,     'uint8_t'),
        c.INFO_DATATYPE_FLOAT64 : (ct.c_double, 'double'),
        c.INFO_DATATYPE_PTR     : (ct.POINTER(ct.c_char), 'uint8_t_ptr'),
        c.INFO_DATATYPE_STRING  : (cE.std_string,         'std_string')
    }
