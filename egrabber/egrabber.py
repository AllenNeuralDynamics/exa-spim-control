...
class _Module():
    ...
    def get_info(self, cmd, info_datatype):
        ...
        info_datatype : int (INFO_DATATYPE)
    ...
    def get(self, feature, dtype=None):
        ...
        dtype : class, optional
            Type of return value. Can either be int, float, bool, str or list.
...
class EGrabber(object):
    ...
    def announce_and_queue(self, memory, **kwargs):
        """Announce and queue a memory buffer from a GenTLMemory, UserMemory, BusMemory or UserMemoryArray object.

        Parameters
        ----------
        memory : GenTLMemory, UserMemory, BusMemory or UserMemoryArray
    ...
    def get_buffer_info(self, buffer_index, cmd, info_datatype):
    ...
    def _get(self, port_handle, module, feature, dtype=None):
        ...
        if dtype in (int, bool):
            ...
            raise TypeError('dtype must be int, float, str, list or None.')
    ...
    def _set(self, port_handle, module, feature, value):
        ...
        if dtype in six.integer_types + (bool,):
            ...
            raise TypeError('value must be int, float, str or bool.')
    ...
    def process_event(self, events='Any', timeout=GENTL_INFINITE):
        ...
        if self._callback_failures[threading.current_thread()] is not None:
...
class BufferIndexRange:
    ...
    def begin(self):
        ...
        if self.size() > 0:
            return self.index_at(self.size() - 1) if self.reverse else self.index_at(0)
        return 0
