...
class WaveformGenerator(object):
	...
	def configure(self, cfg, live = False):
		...
		self.etl_nonlinear = self.cfg.etl_nonlinear
		self.etl_interp_time = self.cfg.etl_interp_time
		self.camera_exposure_time = self.cfg.camera_exposure_time/1000.0
		self.camera_delay_time = self.cfg.camera_delay_time/1000.0
		self.etl_buffer_time = self.cfg.etl_buffer_time/1000.0
		self.laser_buffer_time = self.cfg.laser_buffer_time/1000.0
		self.rest_time = self.cfg.rest_time/1000.0
		self.line_time = self.cfg.dwell_time/1000.0
		self.pulse_time = self.cfg.pulse_time/1000.0
		self.total_time = self.cfg.total_time/1000.0

		self.co_task = nidaqmx.Task('counter_output_task')
		self.co_task.co_channels.add_co_pulse_chan_freq('/Dev1/ctr0', units = nidaqmx.constants.FrequencyUnits.HZ, idle_state = nidaqmx.constants.Level.LOW,  initial_delay = 0.0, freq = 1.0/self.total_time, duty_cycle = 0.5)
		...
		self.ao_task.timing.cfg_samp_clk_timing(rate=self.rate,
												...
												samps_per_chan=int(self.rate*self.total_time))
	...
	def generate_waveforms(self, live = False):
		...
		t0 = t_etl[0]
		t1 = t_etl[int((camera_exposure_samples + etl_buffer_samples)*self.etl_interp_time)]
		tf = t_etl[-1]
		v0 = voltages_etl[0]
		v1 = voltages_etl[int((camera_exposure_samples + etl_buffer_samples)*self.etl_interp_time)] + self.etl_nonlinear
		vf = voltages_etl[-1]
		...
		voltages_t[3, int(etl_buffer_samples/2.0)-laser_buffer_samples+camera_delay_samples:int(etl_buffer_samples/2.0) + camera_exposure_samples + line_time_samples + camera_delay_samples] = 5.0
	...
	def start(self):
		...
		if self.ao_task:
			self.ao_task.start()
		time.sleep(2*self.total_time) # wait for last AO to play
		if self.co_task:
			self.co_task.start()
